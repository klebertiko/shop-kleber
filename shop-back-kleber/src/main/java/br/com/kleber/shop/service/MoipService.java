...
import br.com.kleber.shop.exception.BusinessShopKleberException;
...
public class MoipService {
    ...
    @Autowired
    public MoipService(ObjectMapper objectMapper, @Value("${security.moip.integration.authorizationToken}") String authorizationToken) {
    ...
    private <T> T execute(final String url, final HttpMethod httpMethod, final Object body, final Class<T> responseBody) throws IOException {
        ...
        throw new BusinessShopKleberException(this.objectMapper.writeValueAsString(responseEntity.getBody()));
    }

    private HttpHeaders getHeadersWithAuthorization() {
        HttpHeaders headers = new HttpHeaders();
        headers.add(HttpHeaders.AUTHORIZATION, "Basic " + this.authorizationToken);
